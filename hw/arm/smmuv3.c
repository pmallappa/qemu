...
#define DBG_VERBOSE1 DBG_BIT(DEBUG1)
#define DBG_VERBOSE2 DBG_VERBOSE1 | DBG_BIT(DEBUG1)
#define DBG_VERBOSE3 DBG_VERBOSE2 | DBG_BIT(DEBUG2)
...
#define smmu_evt_irq_enabled(s) __smmu_irq_enabled(s, SMMU_IRQ_CTRL_EVENT_EN)
#define smmu_gerror_irq_enabled(s) __smmu_irq_enabled(s, SMMU_IRQ_CTRL_GERROR_EN)
#define smmu_pri_irq_enabled(s) __smmu_irq_enabled(s, SMMU_IRQ_CTRL_PRI_EN)
...
__smmu_read_aligned_sysmem(SMMUState *s, hwaddr addr, void *buf, int size, int len)
{
...
static SMMUEvtErr
smmu_get_phys_addr(SMMUState *s, SMMUCfg *cfg, Ste *ste, Cd *cd,
                   uint32_t *pagesize, uint32_t *perm, bool stage2, bool is_write)
{
    int     ret, level;
    ...
    outaddr = stage2? &cfg->opa: &cfg->pa; /* Both are same, just for readability */
                                    ...
                                    IOMMUTLBEntry *tlbe, bool is_write)
{
    ...
    if (ste_cfg == STE_CONFIG_S1TR_S2BY || ste_cfg == STE_CONFIG_S1TR_S2TR) {
        ...
        retval = smmu_get_phys_addr(s, s1cfg, ste, cd, &page_size, &perm, false, is_write);
        if (retval != 0) {
        ...
        retval = smmu_get_phys_addr(s, s2cfg, ste, cd, &page_size, &perm, true, is_write);
        if (retval != 0) {
...
smmu_translate(MemoryRegion *mr, hwaddr addr, bool is_write)
{
    ...
    if (IS_DBG_ENABLED(STE)) dump_ste(&ste);
...
smmu_irq_clear_brcm(SMMUState *s, int irq)
{
    ...
    switch(irq) {
    case SMMU_GERROR_CMDQ: val ^= SMMU_INTR_CMD_SYNC; break;
    case SMMU_GERROR_EVENTQ: val ^= SMMU_INTR_EVENT; break;
    case SMMU_GERROR_PRIQ: val ^= SMMU_INTR_PRI; break;
    }
...
static void smmu_irq_clear(SMMUState *s, uint64_t gerrorn)
{
    ...
    /*
     * Only updated GERROR,
     * GERRORN will be updated as part of mmio_write
     */
}
    ...
    while (!SMMU_CMDQ_ERR(s) && !smmu_is_q_empty(s, &s->cmdq)) {
        ...
        default:
            ...
            SMMU_DPRINTF(CRIT, "Unknown Command type: %x, ignoring\n",
                         CMD_TYPE(&cmd));
            if (IS_DBG_ENABLED(CD)) dump_cmd(&cmd);
            break;
        }
                            ...
                            uint64_t val, unsigned size)
{
    ...
    SMMU_DPRINTF(DEBUG2, "reg:%lx cur: %x new: %lx\n", addr,
...
static const SMMUInfo smmu_info[] = { /* ARM Implementation */
    {
        .name = TYPE_SMMU_DEV,
        .desc = "ARM SMMUv3",
        .impl = SMMU_IMPL_ARM,
