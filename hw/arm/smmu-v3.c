                            ...
                            uint64_t val, unsigned size)
{
    ...
    case SMMU_REG_CMDQ_BASE + 4:
        smmu_update_qreg(s, &s->cmdq, addr, addr - SMMU_REG_CMDQ_BASE, val, size);
        return;
    ...
    case SMMU_REG_EVTQ_PROD:
        smmu_update_qreg(s, &s->evtq, addr, addr - SMMU_REG_EVTQ_BASE, val, size);
        return;
    ...
    case SMMU_REG_PRIQ_PROD:
        smmu_update_qreg(s, &s->priq, addr, addr - SMMU_REG_PRIQ_BASE, val, size);
        return;
    }
